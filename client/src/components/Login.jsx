...
const Login = () => {
  ...
  const handleCapture = async () => {
    ...
    try {
      const formData = { username, descriptors: userDescriptor };
      const url = `${window.location.origin}/api/auth/login`; // Replace with your backend endpoint
      const method = "POST";
  ...
  return (
    <div className="bg-black min-h-screen h-full flex flex-col gap-4 items-center px-7">
      <h2 className="text-white text-4xl mt-10 font-medium mb-10">Login</h2>
      <div className="w-full flex flex-col max-w-[500px] mb-2 min-w-[300px]">
        <div>
          <h1 className="text-white text-sm md:text-md lg:text-lg font-medium py-3">Username</h1>
          <input
            type="text"
            className="px-4 py-4 focus:outline-none w-full rounded mb-1 bg-gray-800 text-white"
        ...
        <div className="py-3 flex gap-4 items-center justify-between">
          ...
          <button
            className={`bg-red-600 hover:bg-red-900 min-w-[80px] max-w-[70px] md:max-w-[120px] w-full text-white px-4 rounded-md h-10
      ...
      <div className="flex items-center flex-col gap-1 w-full">
        <button
          className={`bg-red-600 font-medium w-full text-white max-w-[500px] min-w-[300px] py-2 rounded-md px-4 ${
            loading || !userDescriptor || !username ? "opacity-50 cursor-not-allowed" : ""
          }`}
          onClick={handleCapture}
          disabled={!userDescriptor || loading || !username}
        >
          {loading ? "Logging In..." : "Login"}
        </button>
        <p className="text-red-800 p-5">
          <NavLink to="/register">New user? Register your face</NavLink>
        </p>
      </div>
    </div>
  );
